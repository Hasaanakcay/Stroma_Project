...
#define BOOT_FLAG_ADDRESS           0x08004000U // Ön Yükleme Bayrak Adresi
#define APP_ADDRESS                 0x08008000U // Application Adresi
#define TIMEOUT_VALUE               SystemCoreClock/4

#define ACK     0x06U
#define NACK    0x16U

static UART_HandleTypeDef huart; //Huart is called from the HandleTypeDef library.
static uint8_t RX_Buffer[32]; // A 32-bit field is created.
typedef enum
{
    ERASE = 0x43,
    WRITE = 0x31,
...
{
    Clk_Update();
    Boot_Init(); // Function is Started
    
// 
	
    Transmit_ACK(&huart);
    if(HAL_UART_Rx(&huart, RX_Buffer, 2, TIMEOUT_VALUE) == HAL_UART_TIMEOUT) //STM32 UART interface with HAL.
    {
        Transmit_NACK(&huart);
        Jump2App();
    }
    if(Check_Checksum(RX_Buffer, 2) != 1 || RX_Buffer[0] != ACK) //if Buffer size is not equal to 1 or 0th index of Buffer is not equal to ACK. As a result, NACK is transmitted 
    {
        Transmit_NACK(&huart);
        Jump2App();
    }
	...
	// The application address is checked. If there is no problem, it enters the function. First of all, interrupts are disabled. 
}
