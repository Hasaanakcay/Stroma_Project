...
//It has 16 bit data output. Communication is provided using I2C. *

#define RAD2DEG 57.295779513082320876798154814105 //Radians to degrees converter.
#define WHOAMI 0x75 //Read chip registration address
...
uint8_t MPU_Init(I2C_HandleTypeDef *I2Cx)
{
	...
	//Cycle mode off, device active with 8MHz internal clock
        Data = 0;
        HAL_I2C_Mem_Write(I2Cx, MPU6050_ADDR, PWR_MGMT, 1, &Data, 1, I2C_TIMEOUT);
//Sample Rate Divider Set to 7 | Sample Rate = Gyro Output Rate/(1+SMPLRT_DIV)
        Data = 0x07;
...
{
    uint8_t Rec_Data[6];
//Reads Accelometer Measurements
...
{
    uint8_t Rec_Data[14]; 	//Stores I2C Temp Read Values
    int16_t temp;
	//Reads Measurements From Address 0x3B to 0x48
    ...
    double dt = (double)(HAL_GetTick() - timer) / 1000; //Stores the elapsed time since the last call
    timer = HAL_GetTick();
    double roll;
    double roll_sqrt = sqrt( //Roll Calculated in Radiant: Sqrt(Accel_X^2+Accel_Z^2)
    ...
    double pitch = atan2(-DataStruct->Accel_X_RAW, DataStruct->Accel_Z_RAW) * RAD2DEG;
		...
		//Updates Measured Values with Filtered Depending on the Variance
...
//Measuring current angle values
